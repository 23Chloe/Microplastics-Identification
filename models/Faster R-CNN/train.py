...
'''Training expects paired VOC JPEG images and XML annotations with matching class names.'''
if __name__ == "__main__":
    ...
    if total_step <= wanted_step:
        if num_train // Unfreeze_batch_size == 0:
            raise ValueError('The dataset is too small for training; add more samples.')
        wanted_epoch = wanted_step // (num_train // Unfreeze_batch_size) + 1
        print("\n[Warning] When using the %s optimizer, at least %d total training steps are recommended."%(optimizer_type, wanted_step))
        print("\n[Warning] This run has %d training samples, batch size %d, %d epochs, and %d total steps."%(num_train, Unfreeze_batch_size, UnFreeze_Epoch, total_step))
        print("[Warning] Total steps %d are below the recommended %d; consider %d epochs."%(total_step, wanted_step, wanted_epoch))
