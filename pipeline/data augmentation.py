...
parser.add_argument("--seed", type=int, default=0, help="Random seed for reproducible augmentation.")
...
for filename in os.listdir(input_image_folder):
    ...
    for line in lines:
        ...
        if len(parts) != 5:
            raise ValueError(f"Invalid YOLO label row in {label_path}: {line.rstrip()}")
        cls, cx, cy, w, h = int(parts[0]), float(parts[1]), float(parts[2]), float(parts[3]), float(parts[4])
        x1 = (cx - w/2) * width
        y1 = (cy - h/2) * height
        x2 = (cx + w/2) * width
        y2 = (cy + h/2) * height
        bboxes.append(BoundingBox(x1=x1, y1=y1, x2=x2, y2=y2, label=cls))
        if cls == 2:
            has_film = True
    ...
    for i in range(aug_num):
        ...
        with open(out_label_path, "w") as f:
            for bb in bbs_aug.bounding_boxes:
                cls = bb.label
                cx = (bb.x1 + bb.x2) / 2 / aug_width
                cy = (bb.y1 + bb.y2) / 2 / aug_height
                bw = (bb.x2 - bb.x1) / aug_width
                bh = (bb.y2 - bb.y1) / aug_height
